        ...
        <div class="mx-auto max-w-3xl bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
            ...
            <div class="flex items-center justify-between mb-8 border-b border-gray-100 pb-6">
                ...
                @if (isset($producto['id']))
                    <span
                        class="px-3 py-1 text-[10px] font-bold text-solare-musgo bg-solare-musgo/10 rounded-full uppercase tracking-tighter">
                        ID: #{{ $producto['id'] }}
                    </span>
                @endif
            </div>
            ...
            <form action="{{ route('imagenes.store', $producto['id'] ?? '') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div class="group border-2 border-dashed border-gray-200 rounded-xl p-10 text-center transition-all hover:border-solare-musgo hover:bg-gray-50 cursor-pointer"
                        id="dropzone">
                        <div id="dropzone-placeholder" class="flex flex-col items-center">
                            <div
                                class="w-16 h-16 mb-4 rounded-full bg-solare-musgo/5 flex items-center justify-center group-hover:scale-110 transition-transform">
                                <svg class="w-8 h-8 text-solare-musgo" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                            ...
                            <p class="text-xs text-solare-arcilla uppercase tracking-widest mt-1">O haz clic para buscar
                                archivos</p>
                        </div>
                    ...
                    <div class="flex items-center p-4 bg-gray-50 rounded-lg border border-gray-100">
                        <input type="checkbox" name="es_principal" id="es_principal"
                            class="w-4 h-4 text-solare-musgo bg-white border-gray-300 rounded focus:ring-solare-musgo outline-none">
                        <label for="es_principal" class="ml-3 text-sm font-medium text-gray-700">Establecer estas imágenes
                            como principales</label>
                    </div>
                </div>

                <div class="flex gap-3 mt-8 pt-6 border-t border-gray-100">
                    <button type="submit"
                        class="px-8 py-3 text-sm font-bold text-white bg-solare-musgo rounded-lg hover:bg-opacity-90 transition-all shadow-md uppercase tracking-widest">
                        Subir a la Galería
                    </button>
                    <a href="{{ route('productos.index') }}"
                        class="px-8 py-3 text-sm font-bold text-gray-500 bg-gray-100 rounded-lg hover:bg-gray-200 transition-all uppercase tracking-widest">
                        Volver
                    </a>
                </div>
            </form>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-6">
                    @forelse($imagenes as $img)
                        <div class="relative group aspect-square rounded-xl overflow-hidden border border-gray-100 shadow-sm bg-gray-50">
                            ...
                            <!-- Overlay de acciones -->
                            <div
                                class="absolute inset-0 bg-solare-musgo/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-3 backdrop-blur-[2px]">
                                <form action="{{ route('imagenes.destroy', $img['id']) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="p-2.5 bg-white text-red-600 rounded-full hover:bg-red-50 transition-colors shadow-lg"
                                        title="Eliminar">
                                ...
                                @if (!($img['es_principal'] ?? false))
                                    <form action="{{ route('imagenes.principal', $img['id']) }}" method="POST">
                                        @csrf @method('PATCH')
                                        <button type="submit"
                                            class="p-2.5 bg-white text-solare-musgo rounded-full hover:bg-solare-arcilla/10 transition-colors shadow-lg"
                            ...
                            @if ($img['es_principal'] ?? false)
                                <div
                                    class="absolute top-2 left-2 px-2 py-1 bg-solare-musgo text-white text-[8px] font-bold uppercase tracking-widest rounded shadow-sm">
                                    Principal
                                </div>
                            @endif
                        </div>
        ...
        function handleFiles(files) {
            ...
            Array.from(files).forEach(file => {
                ...
                reader.onload = (e) => {
                    ...
                    div.className =
                        'relative aspect-video rounded-lg overflow-hidden border border-gray-200 shadow-sm';
