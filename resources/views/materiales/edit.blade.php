...
@section('label', 'Gestión de Materiales')
@section('header_title', 'Actualizar Información de Insumo')
...
<section class="bg-[#f7f5f2]">
    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16 bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
        <h2 class="serif mb-6 text-3xl font-normal text-gray-900 border-b pb-4">Editar Detalles</h2>
        {{-- @dd($material) --}}
        <form action="{{ route('materiales.update', $id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="descripcion" rows="6"
                        class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-solare-musgo focus:ring-2 focus:ring-solare-musgo focus:border-solare-musgo outline-none">{{ old('descripcion', $material['descripcion']) }}</textarea>
                </div>
            </div>
