            ...
            @forelse($stock as $item)
                ...
                <tr class="hover:bg-[#faf9f7] transition">
                    <td class="px-6 py-4 font-mono text-[11px] text-[#958174] font-bold">
                        SL-{{ str_pad($item['variante_id'] ?? 0, 3, '0', STR_PAD_LEFT) }}
                    </td>
                    <td class="px-6 py-4 font-medium text-gray-900">{{ $item['producto'] ?? 'Sin nombre' }}</td>
                    <td class="px-6 py-4 uppercase text-[10px] tracking-wider">{{ $item['material'] ?? 'General' }}</td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-16 bg-gray-100 h-1 rounded-full">
                                <div class="h-1 rounded-full {{ ($item['stock_total'] ?? 0) > 5 ? 'bg-sage-green' : 'bg-orange-400' }}" 
                                     style="width: {{ min((($item['stock_total'] ?? 0) / 20) * 100, 100) }}%"></div>
                            </div>
                            <span class="font-bold text-gray-900">{{ $item['stock_total'] ?? 0 }}</span>
                        </div>
                    </td>
